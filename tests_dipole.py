from extract_properties_tbfile import get_dipole_orth, k_grid, get_rec_lattice, get_momentum, to_bloch_basis, get_dipole_atomic
import numpy as np
...
def plot_dipole_bz(grid_shape, idxa, idxb, filename, fromFile):
    ...
    fig, axs = plt.subplots(nrows=2, ncols=2, figsize=(33,29))
    p1 = axs[0,0].pcolormesh(X[:,:,0], Y[:,:,0], vec_matrix_re[...,0], 
                            #  shading='gouraud'
                             )
    p2 = axs[0,1].pcolormesh(X[:,:,0], Y[:,:,0], vec_matrix_im[...,0], 
                            #  shading='gouraud'
                             )
    p3 = axs[1,0].pcolormesh(X[:,:,0], Y[:,:,0], vec_matrix_im[...,1],
                            #  shading='gouraud'
                             )
    p4 = axs[1,1].pcolormesh(X[:,:,0], Y[:,:,0], vec_matrix_im[...,1], 
                            #  shading='gouraud'
                             )
    axs[0,0].set_title('Re(d_x)')
    axs[0,1].set_title('Im(d_x)')
    axs[1,0].set_title('Re(d_y)')
    axs[1,1].set_title('Im(d_y)')
    axs[0,0].set_xlabel(r'k_x / $\AA$')
    axs[0,1].set_xlabel(r'k_x / $\AA$')
    axs[1,0].set_xlabel(r'k_x / $\AA$')
    axs[1,1].set_xlabel(r'k_x / $\AA$')
    axs[0,0].set_ylabel(r'k_y / $\AA$')
    axs[0,1].set_ylabel(r'k_y / $\AA$')
    axs[1,0].set_ylabel(r'k_y / $\AA$')
    axs[1,1].set_ylabel(r'k_y / $\AA$')
